...
превью каждого видео. Имя превью каждого видео должно соответствовать имени самого видео.

7. vids.html, vids.css, vids.js - страница видео портфолио

8. Python-скрипт (zvids.py) для автоматического генерирования нового vids.html, если новые видео были помещены в папку vids.

9. Папка sfx для страницы с портфолио саунд-дизайна (sfx.html). В папку помещены все файлы видео для этой страницы, а так же папка prvw, в которую помещены
превью каждого видео. Отличие от vids в том, что некоторые видео в этой папке имеют большой размер (более 100мб), которые не помещаются
в Github Pages. Такие видео помещены в GitHub Releases (https://github.com/Ontnb/ontn.art/releases/tag/SFX). 
Прямые ссылки на помещенные туда видео вставлены в код sfx.html. Превью этих видео помещены в папку sfx/prvw по старому принципу,
изложенному ранее. Скрипт не учитывает этот момент с внешними ссылками, нужно вручную добавить эти прямые ссылки в sfx.html.
Но важно, чтобы на момент генерации скриптом sfx.html в папке sfx находилось видео это видео, иначе скрипт не создаст
соответствующий этому видео блок в sfx.html. 
UPD: видео с GitHub Releases не воспроизводятся с айфонов, поэтому теперь видео хранятся на 
https://dash.cloudflare.com/ (https://dash.cloudflare.com/ba47aa48feff46db63f0fd13363af382/r2/default/buckets/vids). 
Принцип тот же. 

10. sfx.html, sfx.css, sfx.js - страница sfx портфолио

11. Python-скрипт (zsfx.py) для автоматического генерирования нового sfx.html, если новые видео были помещены в папку sfx.
...
Запуск скрипта.
1. Открыть CMD
2. Перейти в дирректорию со скриптами (cd C:\Users\User\Desktop\Site или cd C:\Users\User\Desktop\Site\mobile)
3. Запустить скрипт ("D:\Portable Apps\! Other\Python 3.13.5\python.exe" zsfx.py)
